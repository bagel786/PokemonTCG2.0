...
The retained corpus contains {{decision_rows}} labeled decision cells and {{outcome_pairs}} outcome pairs. For statistically paired claims, both the full framework (B7) and the clustered/hierarchical baseline (B5) detected {{b7_branch_b_detection_pct}}% of hard-invalid cells (n={{b7_branch_b_detection_n}}), but B7 suppressed or downgraded {{b7_branch_b_false_suppression_pct}}% of valid cells while B5 suppressed {{b5_branch_b_false_suppression_pct}}%. Across all branches, B7 detected {{b7_overall_detection_pct}}% of hard-invalid cells and falsely suppressed {{b7_pooled_false_suppression_pct}}% of valid cells, failing both its Branch-C detection requirement and the prespecified 10% pooled false-suppression limit. Results also differed by system: B7 detection was {{b7_holdem_detection_pct}}% in hold'em and {{b7_ising_detection_pct}}% in Ising.
...
The raw-integrity audit passed for {{raw_rows}} total rows (SHA-256 `{{raw_sha256}}`). An independently written script reproduced all Branch-B headline counts without importing the production analyzer.

For Branch B, B7 and B5 each detected {{b7_branch_b_detection_pct}}% of {{b7_branch_b_detection_n}} hard-invalid cells. Their valid-case behavior differed: B7 produced {{b7_branch_b_false_suppression_pct}}% false suppression among {{b7_branch_b_valid_n}} valid cells, whereas B5 produced {{b5_branch_b_false_suppression_pct}}%. The result does not establish equivalence; it shows observed strategy behavior on the frozen bank.

Across branches, B7 hard-invalid detection was {{b7_overall_detection_pct}}%, but Branch-C detection was only {{b7_branch_c_detection_pct}}%, below the predeclared 95% criterion. Pooled false suppression was {{b7_pooled_false_suppression_pct}}%, above the 10% limit. In particular, the implementation downgraded valid Branch-B cases when synchronization evidence was absent even though the conceptual framework allows design-based paired inference without replay or event alignment. This is a reference-implementation failure, not evidence against design-based pairing.
...
Median stored pair runtime was {{holdem_runtime_s}} seconds for hold'em and {{ising_runtime_s}} seconds for Ising. Median stored bytes per pair were {{holdem_storage_bytes}} and {{ising_storage_bytes}}, respectively. An analysis-stage 50-run microbenchmark estimated draw-logging overhead of {{holdem_overhead_pct}}% in hold'em and {{ising_overhead_pct}}% in Ising.
...
The predeclared M1-M2 ordering score had Kendall tau-b {{cross_system_tau}} across the two systems. This score is an ordering device, not a probability or utility. The apparent agreement masks an important magnitude difference: B7 detected {{b7_holdem_detection_pct}}% of hard-invalid cells with {{b7_holdem_fs_pct}}% false suppression in hold'em, versus {{b7_ising_detection_pct}}% detection and {{b7_ising_fs_pct}}% false suppression in Ising.
