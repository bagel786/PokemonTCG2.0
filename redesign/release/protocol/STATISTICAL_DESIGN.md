...
- Interval: Wilson score interval at 95%:
    ...
    q = 1 + z²/n,  center = (p̂ + z²/(2n)) / q,  half = z·sqrt(p̂(1−p̂)/n + z²/(4n²)) / q,  z = 1.959964.

- Clopper–Pearson used where a conservative guarantee is asserted (coverage claims).
- Toy example: x=38, n=40 → p̂=0.950, Wilson CI [0.835, 0.986]. Implemented in `analysis/interval_checks.py` against an independently coded fixture.
