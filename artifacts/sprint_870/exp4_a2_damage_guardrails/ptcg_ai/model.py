...
class NeuralPolicy:
    def __init__(self, path: str | Path, fallback):
        ...
        self.guardrail = GrimGuardrailDirector()
    ...
    def choose(self, obs) -> list[int]:
        ...
        try:
            ranked, desired, _reason = self.guardrail.apply(obs, ranked, desired)
        except Exception:
            pass
        ranked, desired, _ = self.damage_solver.apply(obs, ranked, desired)
        final = sanitize_selection(obs.select, ranked, desired)
        try:
            self.guardrail.commit(obs, final)
        except Exception:
            pass
        return final
