...
def _apply_punk_ac(obs, ranked: list[int], desired: int):
    """Punk activate + count, without target reordering."""
    select = obs.select
    if _effect_id(obs) != MARNIES_GRIMMSNARL_EX:
        return None
    me = obs.current.players[obs.current.yourIndex]
...
class Wave1Rail:
    ...
    def __init__(self, mode: str = "off") -> None:
        ...
        if mode not in {"off", "punk_only", "punk_ac", "punk_count", "punk_target", "tempo", "fan", "floor"}:
