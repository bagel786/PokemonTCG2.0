...
def _apply_attach_tier(obs, ranked: list[int], desired: int):
    """When the policy commits to a hand-energy attach but picks a target outside
    the Grim line while some Grim-line Pokemon still has capacity, prefer the
    Grim-line target by readiness tier.  Reorder-only; never forces an attach."""
    select = obs.select
    if not _context_is(select, int(SelectContext.MAIN), "MAIN") or desired != 1 or not ranked:
        return None
    top = select.option[ranked[0]]
    if int(getattr(top, "type", None)) != int(OptionType.ATTACH):
        return None
    top_target = option_target_pokemon(obs, top)
    if top_target is None or int(top_target.id) in GRIM_LINE:
        return None
    choices = []
    for index in ranked:
        option = select.option[index]
        if int(getattr(option, "type", None)) != int(OptionType.ATTACH):
            continue
        target = option_target_pokemon(obs, option)
        if target is None:
            continue
        card_id = int(target.id)
        if card_id not in GRIM_LINE or _energy_count(target) >= 2:
            continue
        active = _option_active(option)
        deficit = _minimum_attack_deficit(target)
        if active and card_id == MARNIES_GRIMMSNARL_EX:
            tier = 0
        elif not active and card_id == MARNIES_GRIMMSNARL_EX and deficit == 1:
            tier = 1
        elif card_id == MARNIES_MORGREM:
            tier = 2
        elif card_id == MARNIES_IMPIDIMP:
            ...
            continue
        choices.append((tier, -_energy_count(target), _option_slot(option), index))
    if not choices:
        return None
    return _ranked_with(ranked, [min(choices)[-1]]), desired, "attach_grim_tier"
...
class Wave1Rail:
    ...
    def __init__(self, mode: str = "off") -> None:
        ...
        if mode not in {"off", "punk_only", "punk_attach", "tempo", "fan", "floor"}:
    ...
    def apply(self, obs, ranked: list[int], desired: int):
        ...
        elif self.mode == "punk_attach":
            result = (
                _apply_punk_up(obs, ranked, desired)
                or _apply_attach_tier(obs, ranked, desired)
            )
