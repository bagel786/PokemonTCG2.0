...
def _apply_punk_ac(obs, ranked: list[int], desired: int):
    select = obs.select
    if _effect_id(obs) != MARNIES_GRIMMSNARL_EX:
        return None
    me = obs.current.players[obs.current.yourIndex]
...
def _apply_punk_count(obs, ranked: list[int], desired: int):
    select = obs.select
    if _effect_id(obs) != MARNIES_GRIMMSNARL_EX:
        return None
    me = obs.current.players[obs.current.yourIndex]
...
def _apply_punk_target(obs, ranked: list[int], desired: int):
    select = obs.select
    if _effect_id(obs) != MARNIES_GRIMMSNARL_EX:
        return None
    if _context_is(select, int(SelectContext.ATTACH_FROM), "ATTACH_FROM"):
...
class Wave1Rail:
    ...
    def __init__(self, mode: str = "off") -> None:
        ...
        if mode not in {"off", "punk_only", "punk_ac", "punk_count", "punk_target", "punk_ct", "tempo", "fan", "floor"}:
    ...
    def apply(self, obs, ranked: list[int], desired: int):
        ...
        elif self.mode == "punk_ac":
            result = _apply_punk_ac(obs, ranked, desired)
        elif self.mode == "punk_count":
            result = _apply_punk_count(obs, ranked, desired)
        elif self.mode == "punk_target":
            result = _apply_punk_target(obs, ranked, desired)
        elif self.mode == "punk_ct":
            result = (
                _apply_punk_count(obs, ranked, desired)
                or _apply_punk_target(obs, ranked, desired)
            )
