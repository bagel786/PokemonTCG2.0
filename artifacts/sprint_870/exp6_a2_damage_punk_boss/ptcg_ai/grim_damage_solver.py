...
from .card_ids import BOSS_ORDERS, MARNIES_GRIMMSNARL_EX, MUNKIDORI, SHADOW_BULLET
...
class GrimDamageSolver:
    ...
    def apply(self, obs, ranked: list[int], desired: int):
        ...
        try:
            ...
            if effect_id == BOSS_ORDERS and _context(obs, SelectContext.SWITCH):
                return self._solve_boss_target(obs, ranked, desired)
            return ranked, desired, None
    ...
    def _score_shadow(self, target: PublicTarget) -> RouteScore:
    ...
    def _shadow_active_damage(self, obs, card) -> int:
        """Shadow Bullet damage against a defender promoted to the opponent active."""
        opponent = obs.current.players[1 - obs.current.yourIndex]
        if attack_damage_nullified(obs, SHADOW_BULLET, card, opponent, defender_benched=False):
            return 0
    ...
    def _solve_boss_target(self, obs, ranked: list[int], desired: int):
        """Boss's Orders target selection: promote the opponent bench Pokemon
        that maximizes immediate Shadow Bullet prize conversion."""
        if not ranked or desired != 1:
            return ranked, desired, None
        if not self._shadow_ready(obs):
            return ranked, desired, None
        opponent = obs.current.players[1 - obs.current.yourIndex]
        ...
        def boss_score(target: PublicTarget) -> tuple[int, int, int]:
            active_damage = self._shadow_active_damage(obs, target)
            active_ko = active_damage >= target.hp
            prizes = target.prizes if active_ko else 0
            overkill = max(0, active_damage - target.hp) if active_ko else 0
            best_chip = 0
            for other in targets:
                if other.serial == target.serial or other.active:
                    continue
                if other.hp <= 30:
                    best_chip = max(best_chip, other.prizes)
            total = prizes + best_chip
            return (int(total >= remaining), total, -overkill)

        base = boss_score(selected)
        scores = [(boss_score(target), target) for target in targets]
        top = max(score for score, _ in scores)
        if top <= base:
            return ranked, desired, None
        best = min(
            (target for score, target in scores if score == top),
            key=lambda target: target.serial,
        )
        reordered = [best.option_index] + [i for i in ranked if i != best.option_index]
        self._record("boss_exact_conversion")
        return reordered, desired, self.last_intervention
    ...
    def _solve_shadow_target(self, obs, ranked: list[int], desired: int):
        ...
        if selected is None:
            return ranked, desired, None
        base = self._score_shadow(selected)
        scores = [(self._score_shadow(target), target) for target in targets]
