...
from .drag_veto import DragEndEvolveVeto
...
class TargetRouterAgent:
    ...
    def __init__(self) -> None:
        ...
        self.drag_veto = DragEndEvolveVeto(_optional_find("policy_r1.npz"))
        self.errors = 0
    ...
    def __call__(self, obs_dict: dict) -> list[int]:
        ...
        if self.route is None:
            action = self.drag_veto.intercept(obs, action)
            action = sanitize_selection(obs.select, action, len(action))
