...
{{N_CONSTRUCTIONS}} known-truth failures (plus clean controls) inside two open
...
seeds nested inside constructions. The protocol was frozen (commit `{{FREEZE_SHA}}`)
...
detection {{B7_DET_POOLED}} at pooled false suppression {{B7_FS_POOLED}} on
covered cells; simple clustered analysis (B5) attains {{B5_DET_POOLED}} /
{{B5_FS_POOLED}}; method-level acquisition costs differ by
{{COST_SPREAD}}× between cheapest and most expensive strategies; exploratory
common-random-number benefit intervals are reported without independence
assumptions. {{GATE_SENTENCE}} All data, code, freeze records, and deviation
...
of commit `{{FREEZE_SHA}}` plus annotated tag. Repeated-measures cells retain
