#!/usr/bin/env bash
# Priority anti-meta battery (ordered: critical screens first).
set -u
...
run() {
  local name=$1 cand=$2 opp=$3 pairs=$4 seed=$5 heroenv=${6:-'{}'} oppenv=${7:-'{}'}
  ...
  $PY training/evaluate_deterministic_crn.py paired \
    ...
    --candidate "$cand" --control "$E23" --opponent "$opp" \
...
# ---- critical (60 pairs/order) ----
run a_dip_e23_d1   $P/router_dip_e23 "$D1"  60 2026081670 '{"PTCG_TARGET_ROUTE":"force_dipplin"}'
run a_luc_e23_luc2 $P/router_luc_e23 "$LUC2" 60 2026081672 '{"PTCG_TARGET_ROUTE":"force_lucario"}'
run b_router_dip_d1 $P/router_dip_e23 "$D1" 60 2026081674
run p_parity_b0    $P/router_dip_e23 "$B0"  60 2026081678

# ---- secondary (60 pairs/order) ----
run a_dip_e23_d0   $P/router_dip_e23 "$D0"  60 2026081671 '{"PTCG_TARGET_ROUTE":"force_dipplin"}'
run a_luc_e23_luc1 $P/router_luc_e23 "$LUC1" 60 2026081673 '{"PTCG_TARGET_ROUTE":"force_lucario"}'
run b_router_luc_luc2 $P/router_luc_e23 "$LUC2" 60 2026081676
run b_router_dip_d0 $P/router_dip_e23 "$D0" 60 2026081675

# ---- tertiary parity / extra ----
run p_parity_luc_b0  $P/router_luc_e23 "$B0"   60 2026081681
