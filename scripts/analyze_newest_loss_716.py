#!/usr/bin/env python3
"""Fetch all matches and inspect the latest loss against the 716 opponent."""
...
latest_loss = None
for ep in sorted(episodes, key=lambda x: x['id'], reverse=True):
    ag = ep.get("agents", [])
    if len(ag) >= 2:
        h_idx = 0 if ag[0].get("submissionId") == SUB_ID else 1
        if ag[h_idx].get("reward") == -1:
            latest_loss = ep
            break

if latest_loss:
    loss_id = latest_loss['id']
    print(f"\nAnalyzing latest loss: Episode {loss_id}...")
    rp_file = download_replay(loss_id, replays_dir)
