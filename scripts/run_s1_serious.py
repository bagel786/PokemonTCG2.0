#!/usr/bin/env python3
"""S1 serious screen: 200 forced-second + 100 forced-first per opponent."""
...
OUT = ROOT / "artifacts" / "second_order" / "s1_serious"
...
def main() -> int:
    ...
    first = [results[f"{name}_first"].get("win_rate", 0.0) for name in OPPONENTS]
    summary = {
        "second_anchor_macro": sum(second) / len(second),
        "first_anchor_macro": sum(first) / len(first),
        "cells": results,
    }
