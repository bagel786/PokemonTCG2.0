...
def run_once(fork_policy_reset=True):
    if fork_policy_reset:
        fork_policy.reset()
    ...
    while steps < 500:
        ...
        cur = obsd.get('current') or {}
        if cur.get('result', -1) is not None and int(cur['result']) >= 0:
            return int(cur['result']) == hero_seat, steps, trace
        sel = obsd.get('select')
        obs = to_observation_class(obsd)
        acting = int(cur['yourIndex'])
        policy = fork_policy if acting == hero_seat else opp
        action = [int(x) for x in policy.choose(obs)]
        trace.append((steps, acting, int(sel['context']), action))
...
r1 = run_once()
engine.search_release(int(engine.search_begin(harvest_obs, **kwargs)['searchId']))
engine.search_end()
r2 = run_once()
print('run1 win/steps:', r1[0], r1[1])
print('run2 win/steps:', r2[0], r2[1])
if r1[0] != r2[0]:
    t1, t2 = r1[2], r2[2]
    for i, (a, b) in enumerate(zip(t1, t2)):
        if a != b:
            print('first divergence at rollout step', i)
            print('  run1:', a)
            print('  run2:', b)
            break
    else:
        print('no divergence in actions; lengths', len(t1), len(t2))
