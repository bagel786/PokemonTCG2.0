...
# v38: the learned prior DRIVES the MAIN decision; rollout search keeps only the
# verified-lethal override.  Measured on 1000 games of a live 1193 pilot playing our
# exact 60 cards: random 12.8% move agreement, 10s determinized search 14.5-16.6%,
# the prior alone 19.7-22%.  Search is handed the better signal (_nudge <= 6.0,
# clone_bonus +-30) and drowns it in value()'s 1000-per-prize scale.
# Set HERE and not at search.py's default, so every offline harness keeps measuring
# rollout search unless it opts in explicitly.  setdefault => still overridable.
os.environ.setdefault("DIRECT_POLICY", "1")

from agent import playbook  # noqa: E402  -- must import AFTER the flag is set
