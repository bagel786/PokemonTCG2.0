...
def _can_ko_opp_active(state: dict) -> bool:
    ...
    my_type = card_db.pokemon_type(_top_mon(_my_player(state).get("active")).get("id") or 0)
...
_SETUP_SUPPORTER_GUARD = os.environ.get("SETUP_SUPPORTER_GUARD", "0") != "0"


def _setup_supporter_displaced(opt: dict, state: dict, options=None) -> bool:
    """Xerosic burning the setup-turn supporter slot while a productive
    Dawn/Hilda sits in the SAME menu and no Alakazam is in play yet.

    Applied by search's ROOT candidate filter only, deliberately NOT from
    _play_suppressed: inside _play_suppressed it also reshapes policy's rollout
    continuations, and the first gate measured 34/460 changed roots against only
    2 candidate-membership changes -- 16 collateral changes per intended one.
    At the root filter the reach is the predicate's reach.

    2.6a t2-5 exact menus: in LOSSES, 11 of 14 qualifying Xerosic selections had
    Dawn or Hilda legal alongside and only ~14% had the hero in play; in WINS,
    ~91% already had it. Predicate reach on real 2.6a roots: 39 matching states,
    24 semantic changes, ~1.2% of branching MAIN decisions (3.3% at t2-5).

    CAVEAT, not resolved: 2.5a's loss slice had ZERO qualifying Xerosic
    selections, so the loss-side effect is unreproduced in that control. This is
    an experiment arm, not a known fix. Kill with SETUP_SUPPORTER_GUARD=0.
    Turn range and predicate deliberately match scripts/campaign_hypothesis_report.py
    so the shipped guard inherits the measured reach.
    """
    if not _SETUP_SUPPORTER_GUARD or not options:
        return False
    if opt.get("type") != OptionType.PLAY or _play_card_id(opt, state) != _XEROSIC:
        return False
    if not 2 <= (state.get("turn") or -1) <= 5:
        return False
    if any(m.get("id") == ALAKAZAM for m in _in_play(_my_player(state))):
        return False
    return any(o.get("type") == OptionType.PLAY
               and _play_card_id(o, state) in (_DAWN, _HILDA)
               and not _draw_dominated(o, state)
               for o in options)
