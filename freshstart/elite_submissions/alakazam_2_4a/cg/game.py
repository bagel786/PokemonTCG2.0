...
from .sim import Battle, HAS_SEEDED_BATTLE, StartData, lib
...
def battle_start(
    deck0: list[int], deck1: list[int], *, seed: int | None = None
) -> tuple[dict, StartData]:
    """Start the battle.
    ...
    if seed is not None:
        if not HAS_SEEDED_BATTLE:
            raise RuntimeError(
                "this engine does not export BattleStartSeeded; build the local engine"
            )
        if not 0 <= seed <= 0xFFFFFFFF:
            raise ValueError("seed must fit in an unsigned 32-bit integer")
        start_data = lib.BattleStartSeeded(arg, seed)
    else:
