...
from .sim import HAS_SEEDED_AGENT, lib
...
def search_begin(agent_observation: Observation,
    ...
    if "agent_ptr" not in globals():
        search_seed = os.environ.get("CG_SEARCH_SEED")
        if search_seed is not None:
            if not HAS_SEEDED_AGENT:
                raise RuntimeError("CG_SEARCH_SEED requires local AgentStartSeeded")
            agent_ptr = lib.AgentStartSeeded(int(search_seed) & 0xFFFFFFFF)
        else:
