...
SOFT_VETOES = os.environ.get("SOFT_VETOES", "1") != "0"
# 2.4b attrition recalibration (see value()). Defaults reproduce v30 EXACTLY, so
# every offline harness and any follow-up ship keeps measuring v30 unless main.py
# opts in explicitly. ATTRITION_V2=1 re-gates the deck-differential term on how
# close the shallower deck is to running out; ATTRITION_W rescales it.
ATTRITION_V2 = os.environ.get("ATTRITION_V2", "0") != "0"
ATTRITION_W = float(os.environ.get("ATTRITION_W", "5.0"))
# Deck depth at which the differential starts to count, ramping linearly to full
# weight at 0 cards. Only read when ATTRITION_V2 is on.
ATTRITION_D0 = float(os.environ.get("ATTRITION_D0", "25"))
...
def value(state_obs, me: int) -> float:
    ...
    # 2.4b: two measured defects in the v30 form `5.0 * grind * diff`.
    #
    # (a) WRONG GATE. `grind` is 1.0 when nobody has scored -- i.e. on turn 1, when
    #     the differential is 0 -- and decays to ~0.17 by the time 4-5 prizes have
    #     traded. It was strongest where the term is meaningless and off where it
    #     decides the game. A deck edge only converts near the BOTTOM of the deck,
    #     so gate on the shallower deck instead.
    # (b) WRONG SCALE. At 5.0/card a 4-card edge is worth 20 points against
    #     1000.0 * prize_term -- it can never change a decision.
    #
    # Measured 2026-07-28 over 330 of our mirror games and 226 of yushin_1193's
    # (our exact 60 cards on both sides, 29% of our field): mirror games end with
    # BOTH players at 6-9 deck and the end-of-game differential IS the result --
    # his wins 9.0 vs 7.4, his losses 6.1 vs 8.4. At depth 8 the new form prices
    # that 4-card edge at ~163, a quarter of a prize: enough to break ties in a long
    # game, never enough to outrank real prize progress. Note the direction -- this
    # pays us to draw LESS, the opposite of AMMO_W (reward a big hand, -20pp).
    if ATTRITION_V2:
        depth = min(my.deckCount, opp.deckCount)
        endgame = min(1.0, max(0.0, (ATTRITION_D0 - depth) / max(1.0, ATTRITION_D0)))
        attrition = ATTRITION_W * endgame * (my.deckCount - opp.deckCount)
    else:
        attrition = ATTRITION_W * grind * (my.deckCount - opp.deckCount)
...
def _candidates(options: list, hand: list, state: dict | None = None) -> list:
    ...
    if state is not None:
        try:
            if policy_v2.available():
                scores = policy_v2.scores(state, options)
...
def _main_clone_top(options: list, state: dict, k: int = 2) -> list[int]:
    ...
    """
    if not policy_v2.available():
        return []
    try:
        sc = policy_v2.scores(state, options)
    except Exception:  # noqa: BLE001
        return []
...
def _apply_tactical_vetoes(cand: list[int], options: list, state: dict) -> list[int]:
    ...
    try:
        ...
        # Alakazam 2.3b: let search judge the options that were previously
        # removed by heuristic/soft predicates, while retaining every guard
        # backed by a rules fact or a traced catastrophic failure.  Keep this
        # branch structurally identical to the existing all-vetoed fallback so
        # SOFT_VETOES=1 remains exact v30 behavior and the experiment has a
...
def _direct_policy_pick(obs: dict, options: list, cand: list[int], me: int,
    ...
    try:
        if not policy_v2.available():
            ...
            return None
        scores = policy_v2.scores(obs["current"], options)
