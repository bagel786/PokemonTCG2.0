...
- **EXP23:** the PLAY-source binding is enabled and only the option, score,
  count, and value heads are retrained. All three packaged policy files are
...
- **C3 diagnostic:** blind encoding with the same retained data, seed, training
  implementation, and hyperparameters as EXP23.
...
These policies support bounded research on representation aliasing and paired
...
EXP23 and C3 initialize from A2, train one seed for three epochs on retained
feature rows, use AdamW and KL distillation, and freeze all but four heads. The
...
Model behavior may depend on runtime shields not represented in head-only
