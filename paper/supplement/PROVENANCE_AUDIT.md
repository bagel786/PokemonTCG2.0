...
Audit date: 2026-08-23 (America/Chicago). This supplement records what was
...
The requested evidence branch name, `grim-5k-variance-floor`, is not a local
branch ref. The repository instead contains
`archive/grim-5k-variance-floor` at
...
opened. The separate historical branch `final/overnight-20260816` resolves to
...
|---|---|---|---|
| C0 package tree (A2 + Damage V0) | `artifacts/grim_damage_conversion/winner/extracted` | `13426288358d597ead809e45c364c7f7b9274a6eebf55ddd942142e3326535c3` | HASH/CODE-VERIFIED; ignored local tree |
| C0 archive | `artifacts/grim_damage_conversion/winner/grim_a2_damage_v0.tar.gz` | `a44b676f5ca135747b5d4d6923c7fb350a66369d188315b6ac0f291d23ca69e7` | HASH-VERIFIED; ignored local file |
...
`artifacts/grim_damage_conversion/winner/extracted/ptcg_ai/features.py:207`
...
For the affected ordinary PLAY prompts, this aliases distinct hand cards to a
zero source identifier.
...
The retained-row audit in `paper/data/representation_audit.json` (digest
`fd0c0412a65a35f98264993fc5858e869e94ada1ed63e82082ae4023d2673d0a`),
generated by `paper/scripts/audit_representation.py` (digest
`ac4454af12e4ae039ac0a6720c0b59b6dba9f47f27ccefd3628ed59a47d27ead`),
...
contained at least two PLAY identities. These counts are a feature-row/code
...
The manifest also identifies 41,615 eligible rehearsal records after excluding
the held-out teams. The historical training configuration was A2 initialization
and teacher; seed `20260816`; CPU; feature version 2; heads-only updates to
`option_linear`, `score`, `count`, and `value`; AdamW learning rate `1e-4`;
...
`rehearsal_records=0`. The mixer in `training/replay_refresh.py:297` rounds the
...
artifacts, not the paper's preregistered confirmation.

The historical claim “Grim-family pooled approximately +4.5 percentage points
...
|---|---|---|---:|---:|---:|
| `artifacts/final_sprint/exp23_vs_ctl_B0_p1200b.json` | `7fd48c4f0d81ad48baecb98860e578637479943d790cf9ed2e4110ee3a45a551` | `05657c656c04d88b0352bb0db6f9e2a5dce4b234` | 1,200 | +50 | +4.1667 pp |
| `artifacts/final_sprint/exp23_vs_ctl_m1_p800.json` | `fc7870569ec45bc33d2c0ca0c9888339af45febc8f9b9fa1e1577b2358769b79` | `05657c656c04d88b0352bb0db6f9e2a5dce4b234` | 800 | +51 | +6.3750 pp |
| `artifacts/final_sprint/exp23_vs_ctl_m1_p800_fresh.json` | `653eb29775a101228c4fa2a55ca15168dc7fa41430f33f86ddfab5cf468270a4` | `accf274d1594381a0b36768afaea18ea74b74e4c` | 800 | +46 | +5.7500 pp |
| `artifacts/final_sprint/exp23_vs_ctl_rr_p800.json` | `ee1f09a26acc58b4ea1cd78f0f04663f60cb523f8d39a5fafb9e39a4b787c160` | `05657c656c04d88b0352bb0db6f9e2a5dce4b234` | 800 | +50 | +6.2500 pp |
| **Unadjusted total** |  |  | **3,600** | **+197** | **+5.4722 pp** |

The decision packet's same sentence also names an older B0 file and a d842
400-pair cell. Including all six named cells would yield 4,800 nominal pairs,
not 3,600. Moreover,
`exp23_vs_ctl_B0_p800.json` (base seed `202608170100`) and
`exp23_vs_ctl_B0_p1200b.json` (base seed `202608170200`) share 600 paired seeds.
...
pooled as independent pairs. The row-level d842 400-pair artifact cited in the
...
- `artifacts/overnight_20260816/calib_d842_vs_c0.json`;
...
Aug. 13 replay files and can support a newly labeled reanalysis. Such a reanalysis
is a new result; it does not recover the absent historical CERT-B JSON, rows,
...
- `artifacts/grim_b_final_confirmation_vs_d842_second_400pairs.json`, digest
...
`artifacts/grim_sequence_oracle_v0/manifest.json`, digest
...
observed, below the prespecified +3 pp / five-to-six-state gate. The warranted
claim is bounded and inconclusive.
...
Elements” under the binding competition rules. This supplement reports the
...
The public release can reproduce tables and figures from sanitized processed
data, but it cannot make the restricted environment or private replay pipeline
