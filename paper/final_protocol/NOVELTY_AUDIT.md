...
The defensible contribution is their **operational integration into a fail-closed black-box statistical-admission protocol**. In particular, the package binds artifacts and observable boundary seeds, verifies full schedules, checks the complete declared trace projection under identical-arm repetition and multiple execution contexts, distinguishes within-arm repeatability from cross-arm semantic alignment, audits bounded stochastic sources, maps evidence to permitted claims, exercises controlled coupling failures and an event-keyed repair in a synthetic suite, and applies bounded gate logic to a restricted simulator. The generic completed-case taxonomy is a post-acquisition conservative formalization, not part of the frozen experimental plan; it is an executable template that future users must freeze before observing outcomes. The historical audit rule was itself frozen retrospectively after its outcomes existed, while the later experiment-specific acquisition and suppression rules were frozen before the retained prospective results and separately prescribed paired-resampling intervals and secondary McNemar inference.
...
10. **Remaining contribution here:** Boundary-seed identity, complete declared-trace A/A across execution contexts, semantic event-alignment evidence, an executable result-independent admission taxonomy with explicit post-acquisition provenance, controlled pairing failures, and the restricted-engine application.
...
10. **Remaining contribution here:** An implemented and empirically exercised black-box pairing audit, including boundary identities, identical-arm full-projection checks across contexts, the within/cross distinction, a post-acquisition conservative claim taxonomy for future prospective freezing, experiment-specific gate consequences, and controlled coupling-specific failures.
...
7. **CONDITIONAL:** The admission taxonomy must remain executable and result independent, and its post-acquisition provenance plus future prospective-freeze requirement must remain explicit in the rebuilt review package. If it becomes narrative, outcome-contingent, falsely represented as part of the frozen case plan, or absent from the independently runnable artifact, the remaining novelty boundary materially weakens.
