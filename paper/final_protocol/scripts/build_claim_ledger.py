...
def build_rows() -> list[dict[str, str]]:
    ...
    rows = [
        ...
        row(
            "C008A",
            "It did not exist at the protocol commit, cannot be credited as prospective validation for the completed case, and deliberately authorizes less than the frozen plan's inferential outputs.",
            ...
            assumptions="Repository chronology, the frozen paired-resampling and McNemar prescriptions, and explicit formalization metadata are accurate.",
            limitations="The generic engine cannot retroactively become a prospectively frozen artifact or rewrite the historical plan.",
            allowed_wording="post-acquisition conservative executable formalization; frozen experiment-specific plan remains the provenance authority",
        ...
        row(
            "C015",
            "An apparently favorable estimate failed that retrospective control gate, so the rule suppressed the historical paired comparison.",
            ...
            assumptions="The retrospectively frozen repeated-control gate is authoritative for the historical reanalysis.",
            limitations="Suppression does not erase descriptive mismatch diagnostics and is not evidence of prospective blinding.",
        ...
        row(
            "C016",
            "Across \\PreflightUnits{} arm--seed-condition units and \\PreflightExecutions{} execution trajectories, the frozen trace-digest, terminal-outcome, error, and decision-count checks had \\PreflightMismatches{} mismatches.",
            ...
            allowed_wording="zero frozen digest, outcome, error, and decision-count mismatches in tested scope; later byte-count audit also zero",
        ...
        row(
            "C017",
            "Of \\StressClusters{} clusters and \\StressExecutions{} executions, \\StressTraceMismatch{} clusters disagreed on the frozen trace-digest endpoint (\\StressTracePct\\%).",
            ...
            interval_or_test="point result 99/200 on the frozen digest endpoint; separately reported 100,000-draw empirical reweighting quantiles [42.5%, 56.5%]",
            assumptions="All four profiles remain together during resampling.",
            limitations="The later digest-or-byte extension gives the same count; resampling quantiles are fixed-battery sensitivity, not a population CI.",
        ...
        row(
            "C019",
            "The factorial candidate rows were not trace repeated and recorded schedule, boundary seed, outcome, error, and decision count but no trace digests or semantic event identifiers.",
            ...
            allowed_wording="factorial trace digest was not captured and row-level trace repeatability is unavailable",
            forbidden_wording="zero factorial trace mismatches; factorial trace parity",
        ),
        row(
            "C020",
            "Under the actual frozen rule this gate pass authorized the planned seed-matched estimates, 95\\% paired-resampling intervals, and secondary exact McNemar calculation for \\FactorialUnits{} common units per cell (\\FactorialGames{} engine games).",
            ...
            limitations="This states what the frozen plan authorized; the article's later conservative audit declines the inferential labels and does not establish factorial trace repeatability, event alignment, or new-opponent inference.",
            allowed_wording="frozen plan authorized paired estimates, intervals, and a secondary exact McNemar calculation",
            forbidden_wording="fully coupled CRN factorial",
        ),
        row(
            "C020A",
            "Git history orders the binary gate commit before retained result artifacts, and replacing every outcome value while holding gate evidence fixed leaves that gate decision unchanged.",
            ...
            limitations="Git ordering and functional invariance do not establish human blinding or make the post-acquisition descriptive taxonomy prospective.",
        ...
        row(
            "C021",
            "The total fixed-package contrast on that battery was \\PrimaryEstimatePP{} \\pp{}.",
            "Claim-admission consequence", factorial, status="DERIVED_BY_CHECKED_SCRIPT",
            analysis_script=verifier,
            generation_command="python -B paper/final_protocol/scripts/verify_statistics.py",
            analysis_unit="paired unit, equal-weight average across 10 strata", sample_size="2,000", estimate="+0.55 percentage points",
            interval_or_test="the separately stated 100,000-draw paired-unit reweighting quantiles are [-2.05, +3.15] pp",
            assumptions="Frozen schedule and admitted captured-record gate.",
            limitations="Not a population CI or equivalence test.",
            allowed_wording="fixed-battery descriptive estimate with separately labeled empirical reweighting sensitivity",
            forbidden_wording="policy superiority; equality; equivalence",
        ),
        row(
            "C022",
            "The representation, training, and interaction contrasts were \\RepresentationEstimatePP{}, \\TrainingEstimatePP{}, and \\InteractionEstimatePP{} \\pp{}, respectively; the empirical reweighting quantiles for all four contrasts span zero.",
            ...
            interval_or_test="all four empirical reweighting quantile ranges span zero",
        ...
        row(
            "C026",
            "In the restricted-engine case study, the deterministic preflight found zero required trace, outcome, error, or decision-count mismatches across \\PreflightExecutions{} executions.",
        ...
        row(
            "C027",
            "A separate fixed timed-search battery produced complete recorded-trace-projection disagreements in \\StressTraceMismatch{}/\\StressClusters{} seed-condition clusters.",
        ...
        row(
            "C028",
            "Because the restricted engine exposes no semantic event identifiers or event-keyed streams, the case cannot establish cross-arm event alignment.",
