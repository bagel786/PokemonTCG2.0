...
def main() -> int:
    ...
    if ci[0] > 0:
        primary_inference = (
            "The interval excluded zero, providing evidence of improvement within the "
            "frozen seven-opponent population."
        )
    elif ci[1] < 0:
        primary_inference = (
    ...
    macros.write_text(
        ...
        f"\\newcommand{{\\PrimaryEffectPP}}{{{100 * primary['effect']:.2f}}}\n"
        f"\\newcommand{{\\PrimaryCILowPP}}{{{100 * ci[0]:.2f}}}\n"
        f"\\newcommand{{\\PrimaryCIHighPP}}{{{100 * ci[1]:.2f}}}\n"
